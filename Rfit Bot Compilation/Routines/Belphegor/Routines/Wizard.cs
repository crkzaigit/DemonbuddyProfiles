    ...
    public class Wizard
    {
                    ...
                    new Decorator(
                        ...
                        new PrioritySelector(
                            ...
                            new Decorator(ret => ZetaDia.Me.HasBuff(SNOPower.Wizard_Archon),
                                new PrioritySelector(
                                    new SelfCast(SNOPower.Wizard_Archon_SlowTime,
                                        ret => ArchonSlowTimer.IsFinished, ret => ArchonSlowTimer.Reset()),
                                    new SelfCast(SNOPower.Wizard_Archon_ArcaneBlast,
                                        ctx =>
                                            ...
                                            Clusters.GetClusterCount(ZetaDia.Me, ((CombatContext) ctx), 16f) >= 2),
                                    new CastOnUnit(SNOPower.Wizard_Archon_DisintegrationWave,
                            ...
                            new SelfCast(SNOPower.Wizard_WaveOfForce,
                                ctx =>
                                    Clusters.GetClusterCount(ZetaDia.Me, ((CombatContext) ctx), 12f) >= 2 ||
                                    (((CombatContext) ctx).CurrentTargetIsElite)),
                            new SelfCast(SNOPower.Wizard_FrostNova,
                                ctx =>
                                    ...
                                    (((CombatContext) ctx).CurrentTargetIsElite &&
                                     ((CombatContext) ctx).TargetDistance < 16f)),
                            new CastAtLocation(SNOPower.X1_Wizard_Wormhole, ctx => ((CombatContext) ctx).TargetPosition,
                                ctx =>
                                    Clusters.GetClusterCount(((CombatContext) ctx).CurrentTarget, ((CombatContext) ctx),
                                        15f) >= BelphegorSettings.Instance.Wizard.BlackHoleAoECount),

                            //Hydra
                            new CastAtLocation(SNOPower.Wizard_Hydra,
                                ctx => ((CombatContext) ctx).TargetPosition,
                                ctx =>
                                    HydraTimer.IsFinished && !Minion.HasPet(((CombatContext) ctx), Pet.Hydra)
                                    &&
                                    (((CombatContext) ctx).CurrentTargetIsElite ||
                                     Clusters.GetClusterCount(((CombatContext) ctx).CurrentTarget,
                                         ((CombatContext) ctx), 12f) >= BelphegorSettings.Instance.Wizard.HydraAoECount),
                                s => HydraTimer.Reset()),
                            new SelfCast(SNOPower.Wizard_Archon,
                                ctx => ((CombatContext) ctx).CurrentTargetIsElite ||
                                       Clusters.GetClusterCount(ZetaDia.Me, ((CombatContext) ctx), 60f) >=
        ...
        public static void WizardOnLevelUp(object sender, EventArgs e)
        {
            ...
            if (myLevel == 3)
            {
                ...
                Log.Info("Setting Shock Pulse as Primary");
            }
            // Set Shock Pulse-Explosive bolts as primary.
            if (myLevel == 9)
            {
                ZetaDia.Me.SetActiveSkill(SNOPower.Wizard_ShockPulse, 1, 0);
                Log.Info("Changing rune for Shock Pulse: \"Explosive Bolts\"");
            }
            // Set Electrocute as primary.
            if (myLevel == 15)
            {
                ZetaDia.Me.SetActiveSkill(SNOPower.Wizard_Electrocute, -1, 0);
                Log.Info("Setting Electrocute as Primary");
            }
            // Set Electrocute-Chain lightning as primary.
            if (myLevel == 22)
            {
                ZetaDia.Me.SetActiveSkill(SNOPower.Wizard_Electrocute, 1, 0);
                Log.Info("Changing rune for Electrocute: \"Chain Lightning\"");
            }
            ...
            if (myLevel == 2)
            {
                ...
                Log.Info("Setting Ray of Frost as Secondary");
            }
            // Set arcane orb as secondary
            if (myLevel == 5)
            {
                ZetaDia.Me.SetActiveSkill(SNOPower.Wizard_ArcaneOrb, -1, 1);
                Log.Info("Setting Arcane Orb as Secondary");
            }
            // Set arcane orb rune to "obliteration"
            if (myLevel == 11)
            {
                ZetaDia.Me.SetActiveSkill(SNOPower.Wizard_ArcaneOrb, 1, 1);
                Log.Info("Changing rune for Arcane Orb: \"Obliteration\"");
            }
            // ********** SKILL SLOTS 1-4 **********
            ...
            if (myLevel == 9)
            {
                ...
                Log.Info("Setting Wave of Force as slot 2");
            }
            ...
            {
                ZetaDia.Me.SetActiveSkill(SNOPower.Wizard_DiamondSkin, 1, 2);
                Log.Info("Changing rune for Diamond Skin: \"Crystal Shell\"");
                ZetaDia.Me.SetActiveSkill(SNOPower.Wizard_IceArmor, -1, 4);
                Log.Info("Setting Ice Armor as slot 3");
            }
            // Set Wave of Force-Impactful Wave as slot 2
            if (myLevel == 15)
            {
                ZetaDia.Me.SetActiveSkill(SNOPower.Wizard_WaveOfForce, 1, 3);
                Log.Info("Changing rune for Wave of Force: \"Impactful Wave\"");
            }
        ...
        private static readonly WaitTimer ArchonSlowTimer = new WaitTimer(TimeSpan.FromSeconds(5));
